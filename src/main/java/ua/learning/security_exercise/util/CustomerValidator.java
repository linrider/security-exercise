...
public class CustomerValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (customer.getId() != 0) {
            Customer previousVersion = customerService.getById(customer.getId());
            
            if (!previousVersion.getEmail().equals(customer.getEmail())
                    && customerService.findByEmail(customer.getEmail()).isPresent()) {
                errors.rejectValue("email", "This email is already used");
            }

            if (!previousVersion.getUsername().equals(customer.getUsername())
            && customerService.findByUsername(customer.getUsername()).isPresent()) {
